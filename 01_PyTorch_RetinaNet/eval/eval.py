...
def main(args) :
    ...
    for name, param in reference_retinanet.named_parameters():
        if not param.requires_grad:
            print(name, param.numel(), "requires_grad=False")
        else :
            print(name, param.numel(), "requires_grad=True")
    numParams = sum(x.numel() for x in reference_retinanet.parameters())
    print("numParams: ", numParams)
            
    
    # with torch.cuda.device(0) : 
    #     flops, params = get_model_complexity_info(reference_retinanet, (3, 224, 224), as_strings=True, print_per_layer_stat=True, verbose=True, backend='pytorch')
    #     print('{:<30}  {:<8}'.format('Computational complexity: ', flops))
    #     print('{:<30}  {:<8}'.format('Number of parameters: ', params))

    input = torch.randn(1, 3, 224, 224).to(args.device)
    flops, params = profile(reference_retinanet, inputs=(input, ))
...
'''
torchrun eval.py --nproc_per_node=1 --data-path /media/data/coco --batch-size 2  --test-only \
    --weights-path /home/hslee/INU_RISE/01_PyTorch_RetinaNet/train/custom_recipe_model_13.pth \
    --weights /home/hslee/INU_RISE/01_PyTorch_RetinaNet/train/custom_recipe_model_13.pth \
    2>&1 | tee ../logs/flops.txt
'''    