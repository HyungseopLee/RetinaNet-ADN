...
def main(args):
    ...
    if args.test_only :
        ...
        evaluate(model_without_ddp, data_loader_test, device=args.device, skip=skip_cfg_supernet)
        evaluate(model_without_ddp, data_loader_test, device=args.device, skip=skip_cfg_basenet)
        return
...
'''
    ...
    --weights-backbone /home/hslee/INU_RISE/02_AdaptiveDepthNetwork/pretrained/resnet50_adn_model_145.pth \
    2>&1 | tee ./logs2/exp1_goodF_alpha05_noBeta_pyTorchSchedule.txt
    
    (test)
    torchrun --nproc_per_node=1 train_custom.py --dataset coco --data-path=/media/data/coco \
    --model retinanet_resnet50_fpn \
    --batch-size 4 --workers 8 \
    --test-only --weights-path /home/hslee/INU_RISE/03_3_Loss_Develop/pretrained/basemodel_baseline_model_22.pth \
    2>&1 | tee ./logs/basemodel_baseline_test.txt
    
    (baseline)
    torchrun --nproc_per_node=4 train_custom.py \
    --dataset coco --data-path=/media/data/coco \
    --model retinanet_resnet50_fpn --epochs 13 \
    --batch-size 4 --workers 8 --lr-steps 8 11 \
    --world-size 4 --world-size 4 \
    --aspect-ratio-group-factor 3 --lr 0.01 \
    --weights-backbone ResNet50_Weights.IMAGENET1K_V1 \
    2>&1 | tee ./logs/base_model_mmdetection_schedule1x_torchWeight.txt
